...
export async function decodeAudioData(
  ...
  for (let channel = 0; channel < numChannels; channel++) {
    ...
    for (let i = 0; i < frameCount; i++) {
      channelData[i] = dataInt16[i * numChannels + channel] / 32768.0; // Normalize to -1 to 1
    }
  }
  return buffer;
}

// Function to create a PCM Blob suitable for Gemini Live API
export function createPcmBlob(data: Float32Array): { data: string; mimeType: string } {
  ...
  return {
    ...
    mimeType: 'audio/pcm;rate=16000', // Hardcoded for 16kHz
  };
}